...
impl ModelRegistry {
    ...
    pub fn available_models() -> Vec<ModelInfo> {
        ...
        let sense_voice_languages: Vec<String> = vec!["zh", "zh-Hans", "zh-Hant", "en", "yue", "ja", "ko"]
            .into_iter().map(|s| s.to_string()).collect();

        // Parakeet V3 supported languages (25 EU languages)
        let parakeet_v3_languages: Vec<String> = vec![
            "bg", "hr", "cs", "da", "nl", "en", "et", "fi", "fr", "de", "el", "hu", "it", "lv",
            "lt", "mt", "pl", "pt", "ro", "sk", "sl", "es", "sv", "ru", "uk",
        ].into_iter().map(|s| s.to_string()).collect();

        // Canary supported languages
        let canary_flash_languages: Vec<String> = vec!["en", "de", "es", "fr"]
            .into_iter().map(|s| s.to_string()).collect();

        // Cohere supported languages
        let cohere_languages: Vec<String> = vec![
            "en", "fr", "de", "it", "es", "pt", "el", "nl", "pl", "zh", "zh-Hans", "zh-Hant",
            "ja", "ko", "vi", "ar",
        ].into_iter().map(|s| s.to_string()).collect();

        // GigaAM supported languages
        let gigaam_languages: Vec<String> = vec!["ru"].into_iter().map(|s| s.to_string()).collect();
    ...
    /// Get predefined filenames (to skip when discovering custom models)
    pub fn predefined_filenames() -> Vec<String> {
